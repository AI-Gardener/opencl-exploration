...
// This file was slightly modified, especially to test opencl time performance.
...
fn main() -> Result<()> {
    ...
    let vendor_text = match device.vendor_id().unwrap() {
        0x1002 => "AMD",
        0x10DE => "Nvidia",
        0x8086 => "Intel",
        _ => "unknown",
    ...
    let ones: Vec<cl_float> = vec![1.0; ARRAY_SIZE];
    let mut sums: Vec<cl_float> = vec![0.0; ARRAY_SIZE];
    for i in 0..ARRAY_SIZE {
        sums[i] = 1.0 + 1.0 * i as cl_float;
    }

    println!("Vectors creation: {:?}", start.elapsed());
    start = Instant::now();
    ...
    println!("Buffers creation: {:?}", start.elapsed());
    start = Instant::now();
    ...
    println!("Events creation: {:?}", start.elapsed());
    start = Instant::now();
    ...
    let mut results: Vec<cl_float> = vec![0.0; ARRAY_SIZE];

    println!("1 more vec: {:?}", start.elapsed());
    start = Instant::now();
    let read_event =
        unsafe { queue.enqueue_read_buffer(&z, CL_NON_BLOCKING, 0, &mut results, &events)? };

    println!("enqueue read: {:?}", start.elapsed());
    start = Instant::now();

    // Wait for the read_event to complete.
    read_event.wait()?;

    println!("Wait all: {:?}", start.elapsed());
    start = Instant::now();
